...
for i, pos in enumerate(occurrences):
    ...
    context_before = content[max(0, pos-1000):pos]
    context_after = content[pos+len(old_str):pos+len(old_str)+500]
    
    print(f"\nOccurrence {i+1} at position {pos}:")
    print("Context before:", context_before[-200:])
    print("Context after:", context_after[:100])
    
    # Check if this is in SavingsGoalsView by looking for showingAddGoal
    if "showingAddGoal" in context_before and "AddSavingsGoalView" in context_before:
